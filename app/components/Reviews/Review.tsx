...
import { ProductReviewType } from "@/app/types";
const ProductReview1 = ({ reviews }: { reviews: ProductReviewType[] }) => {
	console.log(reviews);
	const [menu, setMenu] = useState(true);
	const [menu1, setMenu1] = useState(false);

    if(!reviews.length) return <p className="text-2xl md:text-3xl font-medium leading-normal text-white text-center">No reviews yet!</p>
	return (
					...
					return ( 
						<div key={index} className='w-full flex justify-start items-start flex-col bg-gray-50 p-8'>
							...
							<div className='flex flex-col md:flex-row justify-between w-full'>
                                ...
                                <p className="text-xl md:text-2xl font-medium leading-normal text-primary">{review.title}</p>
									<button
